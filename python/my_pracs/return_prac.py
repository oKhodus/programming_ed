def done():
    ...
    (
        out.append("#" + m[a - 1])
        if m[a - 1] == "до" or m[a - 1] == "фа"
        else out.append(m[a - 1])
    )
    (
        out.append("#" + m[b - 1])
        if m[b - 1] == "до" or m[b - 1] == "фа"
        else out.append(m[b - 1])
    )
    (
        out.append("#" + m[c - 1])
        if m[c - 1] == "до" or m[c - 1] == "фа"
        else out.append(m[c - 1])
    )
    ...
    for index, char in enumerate(inp):
        if char == "р" and index + 1 < len(inp) and inp[index + 1] == "а":
            out.append(index)
        else:
            continue
    ...
    for index, elem in enumerate(check_row):
        if elem == 0:
            try:
                if (
                    (index_i[index] - index_i[index - 1]) in inc
                    or index_i[index + 1] == index_i[index - 1]
                    or (index_i[index] - index_i[index + 1]) in inc
                ):
                    out.append("НЕТ")
    ...
    class BankAcc:
        ...
        def withdraw(self, amount):
            if self.balance >= amount:
                ...
                print(
                    "oioi, your balance is less than withdraw - please make a deposit"
                )
    ...
    acc = BankAcc(
        input("Please enter your fullname: "),
    ...
    class LinkedList:
        ...
        def display(self):
            ...
            while current_node != None:
                name = f"node_{count}"
                if count == 1:
                    print(f"|list_head-{name}: {current_node.value}|", end=" ")
                else:
    ...
    elem = input("Enter numbers for head: ")
    try:
        while True:
            if elem != "added": 
                ll.insert_head(elem)
    ...
    class Counter:
        ...
        def __str__(self):
            return f"Output: {self.s}"

    c = Counter(3, 6)
    print(*[num for num in c])
    ...
    b1 = Book("1984", "Orwell")
    b2 = Book("1984", "Orwell")
    b3 = Book("Brave New World", "Huxley")
    ...
    s = Sentence("Hello world this is magic methods")
    print(len(s))
    print(
        s[4]
    )
...
class cycleLs:
    ...
    def __str__(self):
        return f"Output: {self.seq}"
    

colors = cycleLs(["red", "green", "blue"])
print(colors)